          ...
          <tbody>
            {playlists.map(p => (
              <tr key={p.id} className="border-b border-zinc-800 last:border-0">
                <td className="text-white px-4 py-3 text-2xl">{getPlaylistIconEmoji(p.icon)}</td>
                <td className="text-white px-4 py-3" style={{ backgroundColor: p.color ?? undefined }}>{p.name}</td>
                <td className="text-white-600 px-4 py-3">{p.description || '—'}</td>
