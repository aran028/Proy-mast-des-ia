...
export default function NewVideoPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      viewCount: parseOptionalNumber(formData.get('viewCount')),
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input name="title" required minLength={3}
            className="w-full bg-zinc-800 border border-zinc-700 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-pink-400" />
        </div>
        ...
        <div>
          <label className="block text-sm text-pink-400 mb-1">Visualizaciones</label>
          <input name="viewCount" type="number" min={0} step={1} placeholder="0"
            className="w-full bg-zinc-800 border border-zinc-700 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-pink-400" />
        </div>
